...
/*   Updated: 2024/11/27 13:30:54 by ccolin           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*replace_substring_with_exit_status(char *str, int start, int end, int exit_status)
{
	char	*prefix;
	char	*variable;
	char	*suffix;
...
static char	*find_end_of_env_key(char *str, t_env_list *envp, int start, int exit_status)
{
	int	end;

	end = start + 1;
	if (str[end] == '?')
			return (replace_substring_with_exit_status(str, start, end, exit_status));
...
=============================================================================*/
char	*find_envps(char *str, t_env_list *envp, int is_command, int exit_status)
{
	int	i;
	int	in_squote;
	int	in_dquote;
	...
	while (str[i])
	{
		...
		{
			str = find_end_of_env_key(str, envp, i, exit_status);
			i = 0;
			in_squote = 0;
			in_dquote = 0;
		}
...
void	expend_envps(t_token *token, t_env_list *envp, int	exit_status)
{
	while (token)
	{
		...
		if (token && (token->type == ENVP || token->type == STRING_TYPE || token->type == DOUBLE_QUOTE))
			token->token = find_envps(token->token, envp, FALSE, exit_status);
		if (token)
			token = token->next;
	}
}
